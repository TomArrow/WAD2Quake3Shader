...
void StudioModel::WritePoints( std::stringstream *ss)
{
	...
	for (i = 0; i < m_pmodel->numverts; i++)
	{
		...
		//(*ss) << "v " << g_pxformverts[i][0] << " " << g_pxformverts[i][1] << " " << g_pxformverts[i][2] << "\n";
		(*ss) << "v " << g_pxformverts[i][0] << " " << g_pxformverts[i][2] << " " << -g_pxformverts[i][1] << "\n";
	}
...
//
	...
	// TA: Duplicated this loop. We first write the texture coordinates. Then later the faces.
	for (j = 0; j < m_pmodel->nummesh; j++) 
	{
		float s, t;
		short		*ptricmds;
