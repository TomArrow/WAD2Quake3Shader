...
// TA: Stripped down version of this file that has no GUI stuff or anything.
// TODO: Get rid of all the rest that doesn't really need to be here, I was just lazy and kept a lot and only removed what wouldn't compile.


const char *g_appTitle = "blah";
...
static int mxMessageBox(void* blah, const char* msg, const char* title, int style = 0) {
	std::cout << msg << "\n";
	return 1;
}
...
ControlPanel::loadModel (const char *filename)
{
	...
	if (g_studioModel.LoadModel ((char *) filename))
	{
		if (g_studioModel.PostLoadModel ((char *) filename))
		{
			...
			mxMessageBox (this, "Error post-loading model.", g_appTitle, NULL);
	}
	else
		mxMessageBox (this, "Error loading model.", g_appTitle, NULL);
}
